...
function freshSlot(): FaceState {
  return {
    ...
    grade: null, condition: null,
    boxes: [],
    imgWidth: 0, imgHeight: 0,
    mode: "upload", videoStream: null,
...
async function getAuthHeaders(): Promise<Record<string, string>> {
  const stored = localStorage.getItem("durian_token");
  ...
  localStorage.setItem("durian_token", access_token);
...
export default function DatasetPageClient() {
  const [slots, setSlots] = useState<Record<string, FaceState>>({
    Trước: freshSlot(), Trái: freshSlot(), Phải: freshSlot(), Sau: freshSlot(),
  });
  ...
  const labels = category === "export_criteria" ? EXPORT_LABELS : CONDITION_LABELS;
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/dataset/stats/`, { headers: authH });
  ...
  // ── Detect ───────────────────────────────────────────────────
  // Called with (file, imgWidth, imgHeight) from InteractiveFaceSlot
  const handleDetect = useCallback(async (face: string, file: File, imgWidth: number, imgHeight: number) => {
    setDetectingSlots((prev) => ({ ...prev, [face]: true }));
    try {
      await runDetect(
        file,
        (boxes: DetBox[], w: number, h: number) => updateSlot(face, { boxes, imgWidth: w, imgHeight: h }),
        (done: boolean) => setDetectingSlots((prev) => ({ ...prev, [face]: !!done })),
        threshold,
      );
    } finally {
  ...
  // ── Save single face (grade + condition → both categories → 8 images) ──
  const handleSaveFace = async (face: string) => {
    const s = slots[face];
    if (!s.file) return;
    if (!s.grade || !s.condition) {
      showToast("Cần chọn cả tiêu chí xuất khẩu và tình trạng thực tế!", "error");
      return;
    }
    if (s.saved) return;
    ...
    try {
      ...
      // Gửi pixel box coords + image dimensions → backend convert sang YOLO format
      fd.append("boxes", JSON.stringify(s.boxes.map(b => ({
      ...
      updateSlot(face, { file: null, boxes: [], grade: null, condition: null, saved: false });
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      for (const face of toSave) {
        ...
        fd.append("grade", s.grade!);
        fd.append("condition", s.condition!);
        fd.append("boxes", JSON.stringify(s.boxes.map(b => ({ x1: b.x1, y1: b.y1, x2: b.x2, y2: b.y2 }))));
        fd.append("img_width", String(s.imgWidth));
        fd.append("img_height", String(s.imgHeight));
        const r = await fetch(`${API_URL}/api/dataset/save-face/`, {
          method: "POST", body: fd, headers: authH,
      ...
      showToast(`Đã lưu ${savedCount}/${toSave.length} face (${savedCount * 8} ảnh)!`, "success");
      loadStats();
      setSlots({
        Trước: freshSlot(), Trái: freshSlot(), Phải: freshSlot(), Sau: freshSlot(),
      });
  ...
  // ── Batch label (current category only) ───────────────────────
  ...
  const totalItems = Object.values(stats).reduce(
    (acc, cat) => acc + Object.values(cat).reduce((a, v) => a + v, 0), 0
  );

  // Count for current category
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "var(--text)", fontFamily: "Outfit, sans-serif", letterSpacing: "-0.01em" }}>
              Thu thập Dataset
            </h2>
        ...
        <div className={styles.headerActions}>
          ...
          <div className={styles.categoryToggle}>
            {(["export_criteria", "condition"] as const).map((cat) => (
              ...
              >
                {cat === "export_criteria" ? "Tiêu chí xuất khẩu" : "Tình trạng thực tế"}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className={styles.thresholdBar}>
        ...
        <input
          type="range" min="5" max="95" value={threshold * 100}
          onChange={(e) => setThreshold(Number(e.target.value) / 100)}
          className={styles.thresholdSlider}
        />
        <div className={styles.thresholdHint}>
          Cao → ít box, chính xác hơn
        </div>
      </div>
      ...
      <div className={styles.batchLabelRow}>
        <span className={styles.batchLabelRowLabel}>
          {category === "export_criteria" ? "Gán tiêu chí hàng loạt:" : "Gán tình trạng hàng loạt:"}
        </span>
