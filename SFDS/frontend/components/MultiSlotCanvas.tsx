...
export default function MultiSlotCanvas({
  ...
  const drawBoxes = useCallback(
    (idx: number, image: string, detections: BoundingBox[]) => {
  ...
  const runDetection = useCallback(
    async (idx: number, file: File, w: number, h: number) => {
      ...
      try {
        ...
        try {
          ...
          updateSlot(idx, {
            isDetecting: false,
            error: 'Không thể kết nối WS server',
          });
          return;
        }
    ...
    (idx: number) => {
      ...
      tc.toBlob(
        (blob) => {
          ...
          const file = new File([blob], `slot-${idx + 1}.jpg`, {
            type: 'image/jpeg',
          });
          updateSlot(idx, {
            image: url,
  ...
  const totalDetections = slots.reduce(
    (s, slot) => s + slot.detections.length,
    0
  );
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          {(['mature', 'immature', 'defective'] as const).map((cls) => {
            const count = slots
              ...
              .filter((d) => d.class_name === cls).length;
            ...
            return (
              <div
                ...
                style={{
                  background: `${colors[cls]}15`,
                  border: `1px solid ${colors[cls]}30`,
                  color: colors[cls],
                }}
              >
                <span
                  className="w-1.5 h-1.5 rounded-full"
                  style={{ background: colors[cls] }}
                />
                {
                  { mature: 'Chín', immature: 'Chưa chín', defective: 'Hư' }[
                    cls
                  ]
                }{' '}
                {count}
        ...
        {slots.map((slot, idx) => {
          ...
          return (
            ...
            >
              ...
              {slot.error && (
                <div
                  className="absolute bottom-2 left-2 right-2 z-10 px-2 py-1 rounded-lg text-xs"
                  style={{
