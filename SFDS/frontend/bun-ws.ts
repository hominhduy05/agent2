...
Bun.serve({
  ...
  websocket: {
    open(ws: any) {
      clients.add(ws);
      console.log(
        `[bun-ws] Client ${ws.data.id} connected (total: ${clients.size})`
      );
    ...
    close(ws: any) {
      clients.delete(ws);
      console.log(
        `[bun-ws] Client ${ws.data.id} disconnected (total: ${clients.size})`
      );
    },
...
async function handleDetect(ws: any, msg: DetectMessage): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[bun-ws] Detection done in ${elapsed}ms — ${filtered.length} results (threshold=${threshold})`
    );
