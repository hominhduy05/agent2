...
export default function AnalyticsPage() {
  ...
  const avgWeight = useMemo(() => {
    const values = events.map((e) => e.weight_kg ?? 0).filter((x) => x > 0);
  ...
  function exportCSV() {
    const header = 'timestamp,grade,class,confidence,weight,camera,event';
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.kpis}>
        ...
        <Card title="AVG WEIGHT" value={`${avgWeight.toFixed(2)} kg`} />
      </section>
      ...
      <div className={styles.content}>
        <div className={styles.panel}>
          ...
          <Bar label="A" value={distribution.A} total={total} color="green" />

          <Bar label="B" value={distribution.B} total={total} color="blue" />

          <Bar label="C" value={distribution.C} total={total} color="orange" />

          <Bar label="D" value={distribution.D} total={total} color="red" />
        </div>
        ...
        <div className={styles.panel}>
          ...
          {Object.entries(cameras).length === 0 ? (
            <div className={styles.empty}>No camera data</div>
          ) : (
      ...
      <section className={styles.tablePanel}>
        <div className={styles.tableHeader}>Recent Detection Events</div>

        {loading ? (
          <div className={styles.empty}>Loading...</div>
        ) : events.length === 0 ? (
          <div className={styles.empty}>No detection history.</div>
        ) : (
              ...
              {events.slice(0, 15).map((e) => (
                <tr key={e.event_id}>
                  ...
                  <td className={styles.td}>
                    {e.weight_kg ? `${e.weight_kg.toFixed(2)} kg` : '-'}
                  </td>
