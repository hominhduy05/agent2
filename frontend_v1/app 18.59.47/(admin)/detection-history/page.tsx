  ...
  return (
    <div className="space-y-6 animate-fade-in p-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2" style={{ fontFamily: 'Sora, sans-serif' }}>
        ...
        <Badge variant="light" color="success" className="py-1 px-3">
          <span className="w-1.5 h-1.5 rounded-full bg-success-500 mr-2 inline-block"></span>
          Đang kết nối
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Metric 1 */}
        <div className="p-5 rounded-2xl border bg-gray-900/40 border-gray-800 backdrop-blur-xl flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Tổng số lượt quét</span>
            <h3 className="text-2xl font-bold text-white mt-1">{metrics.totalScans}</h3>
        ...
        <div className="p-5 rounded-2xl border bg-gray-900/40 border-gray-800 backdrop-blur-xl flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Tỉ lệ quả chín</span>
            <h3 className="text-2xl font-bold text-white mt-1">{metrics.ripeRate}%</h3>
        ...
        <div className="p-5 rounded-2xl border bg-gray-900/40 border-gray-800 backdrop-blur-xl flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Quả sâu hại / Hư</span>
        ...
        <div className="p-5 rounded-2xl border bg-gray-900/40 border-gray-800 backdrop-blur-xl flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Độ tin cậy TB</span>
            <h3 className="text-2xl font-bold text-white mt-1">{metrics.avgConf}%</h3>
            <p className="text-xs text-gray-400 mt-1">
      ...
      {/* FILTER PANEL */}
      <div className="p-6 rounded-2xl border bg-gray-900/40 border-gray-800 backdrop-blur-xl space-y-4">
        <div className="flex items-center gap-2 border-b border-gray-800 pb-3 text-white font-medium">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-gray-400 uppercase">Tìm theo ID / Tên Camera</label>
            <div className="relative">
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 text-sm rounded-lg bg-gray-950 border border-gray-800 text-white focus:outline-none focus:border-brand-500 transition-colors"
              />
          ...
          {/* Camera Select */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-gray-400 uppercase">Lọc theo Camera</label>
            <select
              value={selectedCamera}
              onChange={e => { setSelectedCamera(e.target.value); setCurrentPage(1); }}
              className="w-full px-3 py-2.5 text-sm rounded-lg bg-gray-950 border border-gray-800 text-white focus:outline-none focus:border-brand-500 transition-colors cursor-pointer"
            >
          ...
          {/* Class Select */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-gray-400 uppercase">Phân loại AI</label>
            <select
              value={selectedClass}
          ...
          {/* Date Select */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-gray-400 uppercase">Thời gian quét</label>
            <div className="relative">
              <select
                value={selectedDateRange}
                onChange={e => { setSelectedDateRange(e.target.value); setCurrentPage(1); }}
                className="w-full pl-9 pr-3 py-2.5 text-sm rounded-lg bg-gray-950 border border-gray-800 text-white focus:outline-none focus:border-brand-500 transition-colors cursor-pointer"
              >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2 border-t border-gray-800/40">
          <div className="flex items-center gap-4 w-full max-w-lg">
            <span className="text-xs font-semibold text-gray-400 uppercase whitespace-nowrap">Độ tin cậy tối thiểu:</span>
            <input 
              type="range" 
              min="10" 
              max="95" 
          ...
          <div className="flex justify-end gap-2 text-xs text-gray-500">
            Tìm thấy <strong className="text-white">{filteredData.length}</strong> kết quả nhận diện
          </div>
        </div>
      </div>

      {/* TABLE AND GRID AREA */}
      <div className="rounded-2xl border border-gray-800 bg-gray-900/40 backdrop-blur-xl overflow-hidden">
        {filteredData.length === 0 ? (
          <div className="p-16 text-center space-y-3">
            <Layers className="h-12 w-12 text-gray-700 mx-auto" />
            <h4 className="text-white font-semibold">Không tìm thấy dữ liệu nhận diện</h4>
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-800 bg-gray-950/30 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <div className="w-16 h-12 rounded-lg bg-gray-950 border border-gray-800 overflow-hidden relative flex items-center justify-center">
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex flex-col items-center">
                            <span className="font-bold text-white text-xs">{item.avgConfidence}%</span>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            ...
                            className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-800/80 text-gray-400 hover:text-white hover:bg-brand-500/10 hover:border-brand-500/20 border border-transparent transition-all"
                          >
            ...
            {/* PAGINATION FOOTER */}
            <div className="px-6 py-4 border-t border-gray-800 bg-gray-950/20 flex flex-col sm:flex-row items-center justify-between gap-4">
              <span className="text-xs text-gray-500">
                Hiển thị dòng <strong className="text-white">{(currentPage - 1) * itemsPerPage + 1}</strong> đến{' '}
                <strong className="text-white">
                  {Math.min(currentPage * itemsPerPage, filteredData.length)}
                </strong>{' '}
                trong tổng số <strong className="text-white">{filteredData.length}</strong> kết quả
              </span>
      ...
      <Modal
        ...
        className="max-w-4xl p-6 border border-gray-800 shadow-2xl overflow-hidden"
      >
        {selectedItem && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <div className="md:col-span-3 flex flex-col items-center justify-center bg-gray-950 border border-gray-800 rounded-2xl p-4 min-h-[300px] relative">
              ...
              <div className="md:col-span-2 space-y-6">
                {/* System Specs */}
                <div className="space-y-3 bg-gray-950/40 p-4 border border-gray-800 rounded-xl">
                  ...
                  <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                    <div>
                      ...
                      <strong className="text-white">{selectedItem.cameraName}</strong>
                    </div>
                    <div>
                      <span className="text-gray-500 block">Kích thước ảnh:</span>
                      <strong className="text-white">{selectedItem.imageWidth}x{selectedItem.imageHeight} px</strong>
                    </div>
                    <div>
                      <span className="text-gray-500 block">Thiết bị biên:</span>
                      <strong className="text-white">{selectedItem.device}</strong>
                    </div>
                    <div>
                      <span className="text-gray-500 block">Định dạng Model:</span>
                      <strong className="text-white">{selectedItem.model_format}</strong>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="divide-y divide-gray-800 border border-gray-800 rounded-xl overflow-hidden bg-gray-950/20">
                    {selectedItem.detections.map((det, index) => (
