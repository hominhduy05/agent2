...
function getScaleFit(
  ...
  if (imgAspect > cAspect) {
    dW = clientW;
    dH = clientW / imgAspect;
    dX = 0;
    dY = (clientH - dH) / 2;
  } else {
    dH = clientH;
    dW = clientH * imgAspect;
    dX = (clientW - dW) / 2;
    dY = 0;
  }
  return { dW, dH, dX, dY };
...
function fitWholeFruitBox(
  ...
  const best = [...candidates].sort((a, b) => {
    ...
    const scoreA =
      Math.max(a.confidence, 0.1) +
      Math.min(areaA / (width * height), 0.25) -
      distA;
    const scoreB =
      Math.max(b.confidence, 0.1) +
      Math.min(areaB / (width * height), 0.25) -
      distB;
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (cropBlob) => {
          ...
          resolve({
            file: new File([cropBlob], 'durian-crop.jpg', {
              type: 'image/jpeg',
            }),
            width: w,
            height: h,
...
export default function InteractiveFaceSlot({
  ...
  const availableDevices = devices.filter(
    (dev) =>
      !activeDeviceIds.includes(dev.deviceId) || dev.deviceId === state.deviceId
  );
  ...
  const redrawCanvas = useCallback(() => {
    ...
    const natImg = document.getElementById(
      `img-${face}`
    ) as HTMLImageElement | null;
    const video = videoRef.current;
    const frozenImg = document.getElementById(
      `frozen-${face}`
    ) as HTMLImageElement | null;
    const natW = hasImage
    ...
    for (const box of boxesToDraw) {
      const { cx: dx1, cy: dy1 } = scaleToDisplay(
        box.x1,
        box.y1,
        dX,
        dY,
      ...
      const { cx: dx2, cy: dy2 } = scaleToDisplay(
        box.x2,
        box.y2,
        dX,
        dY,
      ...
      const activeLabel =
        category === 'export_criteria' ? state.grade : state.condition;
  ...
  useEffect(() => {
    if (!isStreaming || frozenFrameUrl || hasImage || state.mode !== 'camera')
      return;
    const video = videoRef.current;
    if (!video) return;
      ...
      await runDetect(
        ...
        (boxes, w, h) => {
          ...
          const stableCluster = stableCandidatesRef.current.filter(
            (candidate) =>
              boxIou(candidate.box, latestCandidate.box) >= STABLE_IOU
          );
          if (stableCluster.length < STABLE_REQUIRED) return;

          const accepted = [...stableCluster].sort((a, b) => {
            const areaA =
              Math.max(0, a.box.x2 - a.box.x1) *
              Math.max(0, a.box.y2 - a.box.y1);
            const areaB =
              Math.max(0, b.box.x2 - b.box.x1) *
              Math.max(0, b.box.y2 - b.box.y1);
  ...
  const getStatusText = () => {
    ...
    if (state.mode === 'camera' && isStreaming && state.file)
  ...
  const getStatusClass = () => {
    ...
    if (state.mode === 'camera' && isStreaming)
      return `${styles.faceStatus} ${styles.readyStatus}`;
    if (!hasImage) return styles.faceStatus;
    if (state.grade && state.condition)
      return `${styles.faceStatus} ${styles.labeledStatus}`;
    return `${styles.faceStatus} ${styles.readyStatus}`;
  };
  ...
  const getStatusDotClass = () => {
    ...
    if (state.grade && state.condition)
      return `${styles.statusDot} ${styles.labeledDot}`;
    return `${styles.statusDot} ${styles.ready}`;
  };
  ...
  return (
    <div className={styles.faceCard}>
      ...
      <div className={styles.controls}>
        ...
        {showDeviceModal && (
          <div
            className={styles.deviceModalOverlay}
            onClick={() => setShowDeviceModal(false)}
          >
            <div
              className={styles.deviceModalCard}
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={styles.deviceText}>
                            ...
                            {used && (
                              <span className={styles.deviceUsed}>
                                Đang dùng
                              </span>
                            )}
                          </span>
        ...
        {hasImage && state.file && (
          <button
            ...
            onClick={() => {
              setDetecting(true);
              onDetect(state.file!, state.imgWidth, state.imgHeight).finally(
                () => setDetecting(false)
              );
            }}
          >
            ...
            {EXPORT_LABELS.map((g) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    onState({ grade: state.grade === g ? null : g })
                  }
          ...
          <div className={styles.labelRow}>
            {CONDITION_LABELS.map((c) => {
              ...
              return (
                <button
                  key={c}
                  className={`${styles.labelBtn} ${isSelected ? styles.labelSelected : ''}`}
                  style={
                    isSelected
                      ? {
                          background: color + '22',
                  ...
                  onClick={() =>
                    onState({ condition: state.condition === c ? null : c })
                  }
                  disabled={state.saved}
                >
                  {c}
...
export async function runDetect(
  ...
  try {
    ...
    formData.append('conf', String(threshold));

    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9000'}/detect/`,
    ...
    const boxes: DetBox[] = (data.detections || []).map(
